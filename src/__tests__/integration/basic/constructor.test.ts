...
describe('Constructor', () => {
    ...
    beforeAll(async () => {
        ...
        contractSrc = fs.readFileSync(path.join(__dirname, '../data/constructor.js'), 'utf8');
        contractIrSrc = fs.readFileSync(path.join(__dirname, '../data/constructor-internal-writes.js'), 'utf8');
        foreignContractSrc = fs.readFileSync(path.join(__dirname, '../data/constructor-helper.js'), 'utf8');
    });
    ...
    const deployContract = async ({ withConstructor = true, withKv = true, addToState = {}, src = contractSrc }) => {
        ...
        const contract = warp.contract<any>(contractTxId).setEvaluationOptions(withConstructor ? { useConstructor: true, internalWrites: !withKv } : {}).connect(wallet);
    ...
    describe('with useConstructor = true', () => {
        describe('0 interactions', () => {
            it('should call constructor on first read state and works with next readStates', async () => {
                ...
                const { cachedValue: { state: state2 } } = await contract.readState();
        ...
        describe('Constructor has access to all smartweave globals', () => {
            ...
            it('should work with KV', async () => {
                ...
                const { cachedValue: kv } = await contract.getStorageValues(
                    ['__init']
                );
        ...
        describe.skip('TODO: doesn"t work internal writes', () => {

            it('should make internal write in constructor', async () => {
                const helperContract = await deployContract({
                    src: foreignContractSrc,
                    withKv: false
                });
                ...
                const contract = await deployContract(
                    {
                        src: contractIrSrc,
                        withKv: false,
                        addToState: { foreignContract: helperContract.txId() }
                    })

                await contract.readState()
                const { cachedValue: { state: externalState } } = await helperContract.readState();
                expect(externalState.counter).toStrictEqual(101);
            });
