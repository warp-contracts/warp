...
export class HandlerBasedContract<State> implements Contract<State> {
  ...
  resetUncommittedState(): void {
    (this.getRoot() as HandlerBasedContract<unknown>)._uncommittedStates = new Map();
  }
  async commitChildStates(interaction: GQLNodeInterface): Promise<void> {
    const uncommittedStates = (this.getRoot() as HandlerBasedContract<unknown>)._uncommittedStates;
    try {
